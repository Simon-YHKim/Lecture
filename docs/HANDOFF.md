...
> 최종 갱신 **2026-09-11 01:45 KST** · Claude Opus 5 (Claude Code) · 커밋은 이 파일의 git 이력 참조

---

## Latest — 2026-09-11 (3차) / 영문판 착수 — 도면·용어·표기를 닫았다 (PR #25 · #26 · #27 · #28)

### 어디까지 왔나

- main HEAD **`90a733d`** · CI 초록 · 열린 PR 0 · 워킹 트리 깨끗.
- 사용자가 이 세션에 정한 것 둘.
  1. **영문판은 영문 도면을 따로 쓴다.** 국문 도면은 검수 중이라 건드리지 않는다.
  2. **영문은 대본·영상 8편까지 끝까지 간다.** (자습만 하고 멈추지 않는다.)
- 머지된 PR 넷 — #25 영문 1단계 · #26 핸드오프 분할 · #27 영문 도면 · #28 표기 인용.
...
| # | 작업 | 크기 | 메모 |
| --- | --- | --- | --- |
| A | **손으로 그린 도해 23개의 영문 짝** | medium | ⭐ 영문판 출고의 마지막 관문. `svg` 필드 안 `<text>` **161개 · 서로 다른 문구 134개** 가 한글이다. `k`/`e` 두 벌로 만들고 **영문이 상자를 넘치는지 브라우저로 재야** 한다 |
| B | 영문 대본 8편 → Zira 합성 → 영상 8편 | large | 국문과 같은 분량. `narrate_tts` 는 Heami·1.38배만 받게 막혀 있어 **영문 음성 정책부터 정해야** 한다 |

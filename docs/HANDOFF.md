...
> 최종 갱신 **2026-09-11 09:15 KST** · Claude Opus 5 (Claude Code) · 커밋은 이 파일의 git 이력 참조
...
- main HEAD **`3de8905`** (PR #43) · CI 초록 · 열린 PR 0 · 워킹 트리 깨끗.
- 공개판 **`autocad-2026.09.11-preview.2`** 게시 — 자습/강의 × 국문/영문 11개 파일.
...
| 강의 · 영문 | `Scripts_EN.zip`(8차시) · `Lecture_EN_L01/L08.mp4` |
...
| C | **영문 강의 슬라이드** | medium | 아래 참조 |
| D | 작도 안내 P2 잔여 5건 | medium | 실제 조작·원본 대조 필요 → A 와 함께 |

### 영문 덱이 왜 아직 없나 (해 보고 되돌린 기록)

`build_deck_all.py` 에 `--root`(차시 폴더 위치) 와 `--lang` 을 달면 영문 사본에서
덱이 만들어진다 — 222장까지 나왔다. 그런데 **본문이 국문 그대로**였다. 자습
원본은 한 마디를 `{ko, en}` 으로 갖고 있고 덱은 `build_deck_selfstudy.ko()`
한 곳에서 국문을 고른다. 거기서 영문을 고르게 바꿨더니 이번에는 빌드가 멎었다 —

    자습본에 「점을 찍는 네 가지 방법」 절이 없다

덱이 **절 제목(한국어 문자열)로 자습본의 절을 찾아** 프레임 뒤에 끼우기 때문이다
(`build_deck_selfstudy.EXTRA`). 언어를 바꾸면 그 대조가 통째로 끊긴다.
제목이 아니라 **절의 id 로 찾도록** 고치는 것이 먼저다. 반쪽짜리 덱을 올리는
것보다 안 올리는 편이 낫다고 보고 두 파일 다 되돌렸다.

...
| 산출물 | 영상 20편 · 248분 | `docs/autocad-technician/self-study/*.html` |
...
### 2-3. 코치 마크가 붙어야 하는 단계

사용자 정책 2번이다. **조작에 `snap` 이 있는 단계**에는 도면과 코치 마크가 있어야
한다. 스냅은 형상 위의 점을 잡는다는 뜻이고, 그 점을 글로만 부르면 「베이스 윗면
왼쪽에서 조금 오른쪽」 같은 말이 되어 화면에서 찾는 일이 학습자에게 남는다.
정책이 없애라고 한 것이 그 탐색이다.

`check-editions.py` 가 그 대응을 센다. 지금 스냅 단계 30개가 전부 붙어 있다.

대화상자·저장·레이어 전환처럼 **도면 위에 짚을 것이 없는 단계**에는 붙이지
않는다. 도면을 띄워 봐야 그 단계가 가리키는 것이 없다.
...
| 8차시 모의 작도 | 없다 | 단계는 있고 **명령은 없다** | 새 명령을 배우는 절이 아니라 2~7차시의 순서를 시간 안에 다시 밟는 절이다. 명령이 비어 있는 것이 맞다 |

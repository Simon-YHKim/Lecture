...
- 연습 형상은 공개 가능한 단일 합성 부품 `EDU-SB-01`로 제작하며, `master-part-geometry.json`의 좌표·크기·개수를 정본으로 사용한다.
- 원본 자료의 주석용 빨강·노랑·파랑은 기본 팔레트로 승격하지 않는다.

## Cumulative drawing language

- 모든 차시는 같은 `EDU-SB-01` 부품과 같은 A3 가로 도면 좌표계를 사용한다.
...
- 아직 배우지 않은 특징은 숨길 수 있지만 이미 완성한 특징의 위치·크기·개수는 바꾸지 않는다. PCD Ø36의 3×Ø4는 시작각 30°, 상부 장공은 중심 (40, 42)·22×8을 유지한다.

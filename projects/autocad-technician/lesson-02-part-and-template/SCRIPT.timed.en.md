...
**Time:** 0:00–0:08
    ...
    `[0:02–0:05]` Lesson 2, the part and the drawing environment.

## Line 2 — The part that holds the wheel that sets belt tension (Frame 2)

**Time:** 0:08–2:08

    `[0:08–0:16]` This is the part you will draw across six lessons. An idler pulley bracket, the kind used on a car engine.

    `[0:16–0:23]` (the front view appears) The drawing on the right is this part seen from the front. Today, rather than drawing, we start by working out what it is.

    `[0:24–0:28]` Knowing what it is for lets you read the drawing faster.

    `[0:29–0:46]` (1 card — what it is for) An idler pulley bracket presses on a belt. You use it to hold tension in a V-belt drive. The wheel itself looks like a pulley. This bracket holds that wheel's shaft. Two bolts fix it to the vertical face of the machine frame.

    `[0:46–1:02]` (2 card — what it really does) A belt stretches with use. Leave it and it slips on the pulley. The motor turns and the other end does not follow. The power is not getting through. Meanwhile friction makes heat, and belt and pulley wear together.

    `[1:02–1:23]` (3 card — where you meet it) For another example, think of a belt conveyor. A bracket like this sits in the run that carries power from the motor to the roller. In maintenance you push the bracket sideways a little to set the tension, then tighten the bolts. That is what the long horizontal holes at the bottom are for.

    `[1:23–1:46]` What do we end up doing in PM when a part like this breaks? First the machine stops and you hunt for a spare. You find the part in the assembly drawing, and if a spare exists you use it. If not, you check the part drawing in a hurry and hand it to the machine shop. What you learn to read today is used right there.

    `[1:46–2:05]` (4 card — what it buys you) With the tension right there is no slip. The power goes through as it should. Belt and pulley wear more slowly, so they last longer. Over-tighten it and the bearing pays instead. Holding the tension at the right place is what this part does.

## Line 3 — Each face is machined differently (Frame 3)

**Time:** 2:08–5:52

    `[2:07–2:20]` Now we look at how the part is made. There is one principle here. **You do not cut every face the same way.** What touches a face decides how it is machined.

    `[2:20–2:33]` (1 row — bore Ø25 H7) Start with the hole in the middle. Diameter 25, grade H7. First you drill it at 24. Then you finish it with a reamer. The surface finish is fine, Ra 1.6.

    `[2:34–2:48]` Why is only this one finished fine? Because a turning shaft sits here. A rough surface lets shaft and bore scrape each other. Friction rises, heat comes with it, and both wear.

    `[2:48–3:10]` Once they wear, the fit you set at the start goes loose. That is to say, the shaft starts to move about. Finished fine, friction drops, the shaft turns smoothly and the part holds its life. Grade H7 cannot be held by a drill alone, so a reaming operation goes in on top.

    `[3:10–3:23]` (2 row — tapped 4-M5, 10 deep) These are the four small holes around the bore. They are the screw seats that hold the shaft cover. You drill them at 4.2. Then you cut the thread with a tap. No surface finish is specified here.

    `[3:23–3:40]` What matters instead is the drill diameter. Too large and only a shallow thread remains. Then the screw cannot take its load. There is a reason the four are evenly spaced, too — so the cover does not tilt to one side.

    `[3:40–3:52]` (3 row — mounting slots 2-R5) These are the two long holes at the bottom. You drill twice at diameter 10. Then you join them with an end mill. Rough is fine here, Ra 12.5.

    `[3:52–4:08]` Why not finish these finely? Because the bolt only passes through. The bolt head and the frame clamp the bracket between them. It does not take load against the wall of the hole. Smooth or rough, the result is the same.

    `[4:09–4:21]` And finishing it finely adds an operation. Finish where it is not needed and all you buy is cost. That judgement shows up directly in the machine shop's quote.

    `[4:22–4:35]` (4 row — boss front face Ø56) This is the round part at the top. It is cut by step milling. The finish is medium, Ra 6.3. The cover lands on this face, so flat is enough. It does not have to be fine.

    `[4:36–4:49]` Why does this part stand forward? The whole plate could have been made thicker. Instead only the material around the shaft, where it is needed, is raised 8 forward.

    `[4:50–5:04]` (5 row — web and outer profile) This is the outer profile of the plate and the web rising through the middle. Profile milling gives it its shape and nothing more. Rough is fine, because these faces touch no other part. They only carry load.

    `[5:05–5:20]` (6 row — fillet R10) This is the inside corner where the web turns into the plate. It comes from the corner radius of the end mill. Why round it? Meet it as a sharp corner and load piles onto that one point. That is where the crack starts.

    `[5:20–5:30]` A radius spreads the load out. This is where the load is greatest on this part. That is why the R is as large as 10.

    `[5:31–5:46]` A real drawing carries more notation than this — surface finish symbols, geometric tolerance, heat treatment. You do not draw those in this course. We will only learn what each symbol asks for, and move on.

## Line 4 — Every shape has a reason (Frame 4)

**Time:** 5:52–7:34

    `[5:49–6:06]` You have seen the machining, so now look at the shape. There is one dangerous habit when reading a drawing: memorising the shape. You have to know why a thing sits where it does. Then you notice when a dimension goes missing.

    `[6:06–6:23]` (1 · base 120 × 16) This is the wide plate at the bottom. It is 120 across, which is wide. The load the shaft above creates twists this plate. Two bolts set far apart are what take that load. Bring them close together and the same load moves it more.

    `[6:23–6:39]` (2 · slot centres 12 apart) The bolt holes are not round. There is a reason they are long. Once the belt is on, you have to push it either way while you watch the tension. That range of adjustment is exactly this 12 millimetres between centres.

    `[6:39–6:54]` (3 · web foot 80 · narrowing upward) Look at the web in the middle. It is wide at the bottom and narrows going up. That is because the bending moment is largest at the foot. Higher up the load is smaller, so you take material away. You put material only where it is needed.

    `[6:55–7:14]` (4 · boss Ø56 · 8 forward) The boss only projects forward. The back is flat. That is because the whole back face has to sit tight against the frame. Project it backwards and only that part touches. The rest lifts off. It is the same eye that finds a clash in an equipment layout before it happens.

    `[7:14–7:27]` (5 · bore Ø25) The centre of the bore is the same as the centre of the boss. What happens if they are off? The load from the turning shaft leans to one side. The thinner side goes first.
...
**Time:** 7:34–9:50
    ...
    `[7:39–7:52]` Why read the dimensions this closely? Because sometimes you have to get a spare made in a hurry. When you do, you hand these numbers to the machine shop as they are. One wrong and the part does not fit.

    `[7:52–8:00]` (1 row) 120 and 90. The outside width and height. How much stock to cut is decided here.

    `[8:00–8:10]` (2 row) 60 and 62. The position of the boss centre. 60 from the left edge, 62 from the bottom. These two fix where the shaft sits.

    `[8:11–8:19]` (3 row) 16. The base height. Not the plate thickness — the height seen from the front. Thickness comes later.

    `[8:19–8:38]` (4 row) 80. The width of the web foot. But there is a radius at that place. The corner is not really there. This is where people get confused. That 80 is measured to the theoretical corner the radius erased. When you draw the web line, you start from that point.

    `[8:39–9:00]` (5 row) The values that fix the slots are 29, 50 and 12. From the left edge to the centre of the first end circle is 29. Between the end circle centres of the two slots is 50. Measure between the two end circles inside one slot and you get 12. That is the range of adjustment we spoke about.
